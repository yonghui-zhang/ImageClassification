...
public class Bp {
    ...
    public double[] hide1_x;//// 输入层即第一层隐含层的输入；hide1_x[数据的特征数目+1]， hide1_x[0]为1
    public double[][] hide1_w;// 隐含层权值，hide1_w[本层的节点的数目][数据的特征数目+1];hide_w[0][0]为偏置量
    public double[] hide1_errors;// 隐含层的误差,hide1_errors[节点个数]

    public double[] out_x;// 输出层的输入值即第二次层隐含层的输出 out_x[上一层的节点数目+1]， out_x[0]为1
    public double[][] out_w;// 输出层的权值 hide1_w[节点的数目][上一层的节点数目+1]//
    // out_w[0][0]为偏置量
    public double[] out_errors;// 输出层的误差 hide1_errors[节点个数]

    public double[] target;// 目标值，target[输出层的节点个数]
